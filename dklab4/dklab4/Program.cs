...
class Lab4
{
	...
	private class Version
	{
		...
		{
			Console.WriteLine("Created: Denis Kosenko IPS-43");
	...
	private class Run
	{
		private int OnExecute(IConsole console)
		{
			console.Error.WriteLine("Choose:");
			return 1;
		}
		...
		private abstract class LabsLibrary
		{
			...
			protected string GetInputPath()
			{
				...
				if (string.IsNullOrWhiteSpace(actualPath))
				{
					actualPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
				}
				actualPath = Path.Combine(actualPath, "INPUT.TXT");
			...
			protected string GetOutputPath()
			{
				...
				{
					return Path.Combine(actualPath, "OUTPUT.TXT");
				}
